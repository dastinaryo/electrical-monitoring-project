...
#define API_KEY "API_KEY_PROJECT_FIREBASE"

// Insert RTDB URLefine the RTDB URL */
#define DATABASE_URL "DATABASE_PROJECT_FIREBASE" 
...
void setup() {
  ...
  xTaskCreate(firebaseTask, "Firebase Task", 65536, NULL, 2, &firebaseTaskHandle);
  xTaskCreate(servoTask, "Servo Task", 20000, NULL, 3, &servoTaskHandle);
}
...
void pzemTask(void *parameter){
  for (;;) {
    ...
    ESP_LOGI("PZEM-004T", "Energy :  %.3f kWh", energy1);
...
void servoTask(void *parameter){
  for (;;) {
    ...
    ye1 = myservo.write(servoPin[0], pos, speed, ke);
    vTaskDelay(10000000);
  }
}